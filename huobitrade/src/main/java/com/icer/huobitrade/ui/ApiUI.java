...
public class ApiUI extends BaseUI implements View.OnClickListener {
...
//        App.getApp().setSymbol("eosusdt");
    ...
    protected void initView() {
        super.initView();
        getSupportActionBar().setTitle("接口调试");
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_set_secret: {
                ...
                id.show("设置Secret", "请输入你的Secret", new InputDialog.Callback() {
                    ...
                    @Override
                    public void onOk(String input) {
                        App.getApp().setSecret(input);
                    }
                });
            }
            break;
            case R.id.btn_kline: {
                ReqMarket.getKLine(App.getApp().getSymbol().getSymbol(), ReqMarket.Period.MIN, 150, new Request.EntityCallback<KLineResp>(KLineResp.class) {
                    @Override
            ...
            case R.id.btn_ticker: {
                ReqMarket.getTickerDetail(App.getApp().getSymbol().getSymbol(), new Request.EntityCallback<TickerResp>(TickerResp.class) {
                    @Override
            ...
            break;
            case R.id.btn_depth: {
                ReqMarket.getDepth(App.getApp().getSymbol().getSymbol(), ReqMarket.DepthType.STEP0, new Request.EntityCallback<DepthResp>(DepthResp.class) {
                    @Override
            ...
            break;
            case R.id.btn_trade: {
                ReqMarket.getTrade(App.getApp().getSymbol().getSymbol(), new Request.EntityCallback<TradeResp>(TradeResp.class) {
                    @Override
            ...
            case R.id.btn_trade_history: {
                ReqMarket.getTradeHistory(App.getApp().getSymbol().getSymbol(), new Request.EntityCallback<TradeHistoryResp>(TradeHistoryResp.class) {
                    @Override
            ...
            case R.id.btn_market: {
                ReqMarket.getMarket24H(App.getApp().getSymbol().getSymbol(), new Request.EntityCallback<MarketResp>(MarketResp.class) {
                    @Override
                ...
                for (Account account : App.getApp().getAccount()) {
                    if (account.isMarginAccount()) {
                        ReqOrder.place(account.getId() + "", "1", "1", true,
                                App.getApp().getSymbol().getSymbol(), ReqOrder.OrderType.BUY_LIMIT,
            ...
            case R.id.btn_orders: {
                ReqOrder.orders(App.getApp().getSymbol().getSymbol(), ReqOrder.OrderState.FILLED,
            ...
            case R.id.btn_matchs: {
                ReqOrder.matchResults(App.getApp().getSymbol().getSymbol(),
